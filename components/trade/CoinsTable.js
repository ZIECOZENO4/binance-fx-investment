  ...
  return (
    <nav>
      <ul className="inline-flex -space-x-px">
        {pageNumbers.map(number => (
          <li key={number} className="px-4 py-2 border-gray-300 border">
            <a onClick={() => paginate(number)} href="#!" className={`block py-2 px-3 leading-tight ${currentPage === number ? 'bg-blue-500 text-white' : 'bg-white text-blue-500'} cursor-pointer`}>
              {number}
            </a>
          </li>
        ))}
...
export default function CoinsTable() {
  ...
  return (
    <div className="text-center dark:bg-gray-800">
      <h4 className="text-2xl font-bold my-4 text-gray-200">Cryptocurrency Prices by Market Cap</h4>
      <input className="mb-5 p-2 w-full border rounded border-gray-300" placeholder="Search For a Crypto Currency.." onChange={(e) => setSearch(e.target.value)} />
      {loading ? (
        <div className="bg-yellow-400 p-4">Loading...</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="table-auto w-full">
            ...
            <tbody>
              {currentCoins.map((coin) => {
                ...
                return (
                  <tr key={coin.id} className="bg-gray-700 text-gray-200 cursor-pointer hover:bg-gray-600" onClick={() => router.push(`/dashboard/trade/tradePage/${coin.id}`)}>
                    <td className="px-4 py-2 flex items-center gap-4">
                      <img src={coin.image} alt={coin.name} className="w-12 h-12" />
                      <div>
                        <p className="uppercase font-bold">{coin.symbol}</p>
      ...
      <Pagination itemsPerPage={coinsPerPage} totalItems={handleSearch().length} paginate={paginate} currentPage={currentPage} />
    </div>
  );
}
