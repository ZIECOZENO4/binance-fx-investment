...
const Invest: React.FC = () => {
  ...
  return (
    <div className='text-white h-[400vh]  md:h-[400vh]'>
    ...
    <Note />
    <div>
      <div className=" mt-10 md:mt-20 bg-gradient-to-r  from-blue-600 to-sky-500 flex-row flex justify-between align-middle items-center p-4">
...
<SignedIn>
        { isSignedIn && <div className=" flex text-header text-md px-2 md:text-2xl font-bold font-sans gap-3 text-white uppercase"> INVESTOR  <br />  {user && user.firstName ? user.firstName : user ? user.username : "FX User "}</div> }
              </SignedIn>
      <SignedOut>
...
<p className=' md:text-2xl text-xl font-bold font-sans gap-3 text-white'>BALANCE :<br /> { userBalance !== null ? `$${formatWithCommas(userBalance.toFixed(2))}` : '$ 0.00'}</p>
</div>
<div>

</div>
      </div>
    <div className='flex flex-row px-2 md:px-[20vw] justify-between  w-[100vw] md:w-[60vw] select-none items-center gap-3 rounded-lg border border-sky-500 py-3  text-center align-middle font-sans text-xs font-bold uppercase text-sky-500 transition-all hover:opacity-75 focus:ring focus:ring-sky-200 active:opacity-[0.85] disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none
 mt-5 md:mt-10'>
      <motion.header
